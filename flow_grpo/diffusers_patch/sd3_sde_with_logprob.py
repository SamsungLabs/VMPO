...
def pred_orig_latent(transformer, pipeline, latents, timesteps, embeds, pooled_embeds, config):
    ...
    timesteps = timesteps.clone()   # to avoid modifying the original timesteps
    end_mask = timesteps == 0
    timesteps[end_mask] = scheduler.timesteps[-1]   # to avoid indexing error

    step_index = [scheduler.index_for_timestep(t) for t in timesteps]
    sigma = scheduler.sigmas[step_index].view(-1, *([1] * (len(latents.shape) - 1)))

    sigma[end_mask] = pipeline.scheduler.sigmas[-1] # pipeline.scheduler.sigmas[-1] -> 0.0

    pred_original_sample = latents - sigma * noise_pred # predicted x_0 in paper
